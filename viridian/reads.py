    ...
    def write_reads_files_one_amp(self, amplicon_index, fragments):
        ...
        if paired:
            ...
            if stats["total_reads"] > 0:
                fwd_percent = round(
                    100 * stats["total_reads_fwd_strand"] / stats["total_reads"]
                )
                if not 10 <= fwd_percent <= 90:
                    logging.warning(
                        f"Reads for amplicon {amplicon['name']} mostly all mapped to one strand. {fwd_percent}% mapped to forwards strand"
                    )
