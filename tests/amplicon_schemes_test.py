...
def test_get_built_in_schemes():
    found_schemes = amplicon_schemes.get_built_in_schemes("sars-cov-2")
...
def test_load_list_of_amplicon_sets():
    with pytest.raises(Exception):
        amplicon_schemes.load_list_of_amplicon_sets(
            "sars-cov-2", built_in_names_to_use=None, tsv_others_to_use=None
        )
    ...
    with pytest.raises(Exception):
        amplicon_schemes.load_list_of_amplicon_sets(
            "sars-cov-2", tsv_others_to_use=tmp_tsv
        )
    ...
    with pytest.raises(Exception):
        amplicon_schemes.load_list_of_amplicon_sets(
            "sars-cov-2",
            built_in_names_to_use={"does not exist"},
    ...
    got_dict = amplicon_schemes.load_list_of_amplicon_sets(
        "sars-cov-2",
        built_in_names_to_use={"COVID-ARTIC-V4.1"},
        tsv_others_to_use=tmp_tsv,
    )
    assert got_dict == expect_dict
    os.unlink(tmp_tsv)
